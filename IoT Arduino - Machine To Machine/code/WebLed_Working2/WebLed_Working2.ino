      ...
      if (client.available()) {
        ...
        if (c == '\n') {
          ...
          for(int pa = 0; pa < pressCount; pa++){
           ...
           client.println(" <script> function myFunction(){document.getElementById(\"barRange\").value = \"75\";}</script>");

          for(int pinD = 3; pinD < 6; pinD++){
            client.println("<script>function myFunction");
            client.println(pinD);
            client.println("(){ var x = document.getElementById(\"barRange");
            client.println(pinD);
            client.println("\").value; document.getElementById(\"affichage\").innerHTML = x;}</script>");
          }
          
           client.println("<script>function myFunction(){ var x = document.getElementById(\"barRange\").value; document.getElementById(\"affichage\").innerHTML = x;}</script>");
...
void InputBar(EthernetClient cl, int pin)
{
  
  cl.println("<input type=\"range\" id=\"barRange");
   cl.println(pin);
  cl.println("\">");

  cl.println("<button onclick=\"myFunction");
  cl.println(pin);
  cl.println("()\">Save</button>");
  cl.println("<p id=\"affichage\"></p>");
}

void InputButton(EthernetClient cl, int pin)
{
  cl.println("<button onclick=\"myFunction()\">Save</button>");
  cl.println("<p id=\"affichage\"></p>");
}
